...
describe("Terminal reconnect", () => {
  ...
  it("reconnects immediately after a visible-tab socket close", () => {
    render(<Terminal worktree="feature/reconnect" terminalTheme={getTheme("github-dark").terminal} />);
  ...
  it("only retries once automatically for a visible-tab close", () => {
    render(<Terminal worktree="feature/retry-once" terminalTheme={getTheme("github-dark").terminal} />);
  ...
  it("waits for the tab to become visible before reconnecting hidden closes", () => {
    render(<Terminal worktree="feature/visible" terminalTheme={getTheme("github-dark").terminal} />);
  ...
  it("applies theme updates to the terminal instance", () => {
    const initialTheme = getTheme("github-dark").terminal;
    const nextTheme = getTheme("github-light").terminal;
    const { rerender } = render(<Terminal worktree="feature/theme" terminalTheme={initialTheme} />);

    const terminal = MockTerminal.instances[0]!;
    expect(terminal.options.theme).toBe(initialTheme);

    rerender(<Terminal worktree="feature/theme" terminalTheme={nextTheme} />);

    expect(terminal.options.theme).toBe(nextTheme);
  });

  it("renders stale terminal refresh controls inside the terminal surface", () => {
    ...
    render(
      <Terminal
        ...
        onrefreshagentterminal={onrefreshagentterminal}
      />,
    );
