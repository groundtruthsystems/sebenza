...
import MobileChatSurface from "./MobileChatSurface";
import {
...
async function emitStreamEvent(event: AgentsUiConversationEvent): Promise<void> {
...
describe("MobileChatSurface", () => {
  beforeEach(() => {
    // `shouldAdvanceTime` keeps the fake clock tracking real time so Testing
    // Library's findBy/waitFor polling still fires (it cannot auto-detect
    // vitest fake timers), while advanceTimersByTimeAsync still jumps the clock.
