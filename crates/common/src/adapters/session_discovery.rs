use crate::adapters::{claude_cli, codex_session_log, grok_session_log};
...
pub fn list_session_ids(agent: DiscoverableAgentKind, cwd: &str) -> Vec<String> {
    match agent {
        ...
        DiscoverableAgentKind::Grok => grok_session_log::list_session_ids(cwd),
...
/// pinned (Codex). Claude and grok pin theirs, so this is only their safety net.
/// Returns `None` if nothing new appears within the retry budget.
